      ...
      <ul className="space-y-4">
        {entries.map((entry) => (
          <li key={entry.message} className="border border-gray-200 dark:border-gray-700 p-4 rounded-md shadow-sm bg-white dark:bg-gray-900">
            ...
            <div className="mt-4 p-4 rounded-md border-[1px] border-[#2A2A2A] text-base text-[#2A2A2A] dark:text-slate-200 bg-slate-50 dark:bg-[#2A2A2A]">{entry.message}</div>
            <div className="font-noto my-2 text-sm text-gray-500 dark:text-gray-500">{dayjs(entry.date).format("YYYY-MM-DD HH:mm:ss")}</div>
            {entry.answer && (
              <div className="mt-4 p-4 rounded-md text-base text-right text-slate-800 bg-[#FFE00C]">{entry.answer}</div>
            )}
          </li>
        ))}
      </ul>
